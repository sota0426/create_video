...
def main(root_file_path):
    # Set up
    excel_file_path = r"C:\Users\User\Desktop\english\作成python\スライド作成マクロ_JPN.xlsm"
    sheet_name = "Sheet2"
    ...
    file_path_female = os.path.join(root_file_path, "audio", "JPN_female")
    for loop in range(1, 4):
        input_folder_i = os.path.join(file_path_female, str(loop))
        os.makedirs(input_folder_i, exist_ok=True)
        print("フォルダ作成 id=" + str(loop))

    file_path_male = os.path.join(root_file_path, "audio", "JPN_male")
    for loop in range(1, 4):
        input_folder_i = os.path.join(file_path_male, str(loop))
        os.makedirs(input_folder_i, exist_ok=True)
        print("フォルダ作成 id=" + str(loop))
    ...
    for row_num, row in enumerate(sheet.iter_rows(min_row=2), start=2):  # Assume header is in the first row
        for i in range(1, 4):
            ...
            path_text1 = os.path.join(file_path_female, str(i), f'{row[0].value}.mp3')
            path_text2 = os.path.join(file_path_male, str(i), f'{row[0].value}.mp3')
#            if os.path.exists(path_text1) :
 #               print(f"音声ファイル {row[0].value}.mp3 は既に存在します。スキップします。")
  #              continue
            ...
            Text=text_value,  # row[i].value から text_value に変更
            OutputFormat='mp3',
            VoiceId=voice_ids[1],  # text1 voice
            Engine='neural',
            )

            # Save text1 audio
            with open(path_text2, 'wb') as file_text1:
                file_text1.write(result_text_no1['AudioStream'].read())
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        # コマンドラインからの呼び出しの場合
        data_input = r"C:\Users\User\Desktop\english\作成python"
        main(data_input)